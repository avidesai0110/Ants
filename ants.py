...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if place.ant.is_container == True and place.ant.ant_contained == None and self.is_container == False:
                place.ant.ant_contained = self
            elif self.is_container == True and self.ant_contained == None and place.ant.is_container == False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def helper(input, upperbound, lowerbound):
            if upperbound >= 0 and lowerbound <= 0:
                if input.bees != [] and not input.is_hive:
                    return random_bee(input.bees)
            if input.entrance:
                return helper(input.entrance, upperbound - 1, lowerbound - 1)
            else:
                return None
        return helper(self.place, self.upper_bound, self.lower_bound)

        """
        def helper(input, count):
            if count > self.upper_bound:
                return None
            if count < self.lower_bound:
                return None
            if count == self.lower_bound:
                return helper(self.place, count + 1)
            if input.bees != [] and not input.is_hive:
                return random_bee(input.bees)
            if input.entrance:
                return helper(input.entrance, count + 1)
            else:
                return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount <= 0:
            ...
            inflicted_damage = self.damage + amount
            for eachbee in copy:
                eachbee.reduce_health(inflicted_damage)
            super().reduce_health(amount)
            
        else:
            copy = self.place.bees[:]
            for eachbee in copy:
                eachbee.reduce_health(amount)
            super().reduce_health(amount)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew == 0:
            unluckybee = random_bee(self.place.bees)
            if unluckybee:
                unluckybee.reduce_health(unluckybee.health)
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        if self.ant_contained != None:
            ...
            copy = self.place.bees[:]
            for eachbee in copy:
                eachbee.reduce_health(self.damage + 1)
            '''
        copy = self.place.bees[:]
        for eachbee in copy:
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.queen_construct == True:
            return None
        else:
            gamestate.queen_construct = True
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        target.remaining_slow = 5

        # BEGIN Problem EC
        def slowing(gamestate): 
            if target is not None:
                if target.remaining_slow != 0:
                    target.remaining_slow -= 1
                ...
                #if self.slowing_down == True:
                #self.remaining_slow -= 1
                #if self.remaining_slow == 0:
                    #self.slowing_down = False
        target.action = slowing

        '''
         if self.slowed_down == True: 
            self.remaining_slow -= 1
            if self.remaining_slow == 0:
